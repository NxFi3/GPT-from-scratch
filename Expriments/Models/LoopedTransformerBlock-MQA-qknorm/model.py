...
class GPTModel(nn.Module):
    def __init__(self, vocab_size:int, max_len:int, d_model:int, num_heads:int, d_ff:int, num_loops:int, dropout=0.1):
        super().__init__()
    ...
    def forward(self, x, mask=None, padding=None, custom_num_loop=None):
        x = self.embedding(x) 
        
        loops = custom_num_loop if custom_num_loop is not None else self.num_loops
        
        for _ in range(loops):
            x = self.blocks(x, mask, padding) 
